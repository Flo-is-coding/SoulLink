...
// Key = attacking type, Value = { type: multiplier }
// Only stores super effective (2) and not very effective (0.5) and immune (0)
...
// Calculate defensive effectiveness for a Pokemon with given types
...
export function getDefensiveMatchups(types: string[]): {
  ...
  for (const atkType of Object.keys(TYPE_COLORS)) {
    ...
    for (const defType of types) {
      const eff = EFFECTIVENESS[atkType]?.[defType];
      if (eff !== undefined) {
        mult *= eff;
      }
    }
